...
it('uses a dynamic mL maximum and revalidates size changes and mode switches', async () => {
 const { user, onSave } = await setup()
 await user.selectOptions(screen.getByLabelText('Serving size / volume'), 'custom')
 fireEvent.change(screen.getByLabelText('Custom volume (mL)'), {target:{value:'700'}})
 fireEvent.change(screen.getByLabelText('Number of servings consumed'), {target:{value:'10.1'}})
 ...
 expect(ml).toHaveAttribute('max','7000')
 expect(ml).toHaveValue(7070)
 fireEvent.submit(screen.getByRole('button',{name:'Record Drink'}).closest('form')!)
 expect(onSave).not.toHaveBeenCalled()
 fireEvent.change(ml,{target:{value:'7000'}})
 ...
 expect(ml).toHaveValue(7000)
 expect(ml).toHaveAttribute('max','3500')
 expect(screen.getByRole('button',{name:'Record Drink'})).toBeDisabled()
 expect(screen.getByText(/Enter no more than 3500 mL/)).toBeInTheDocument()
 fireEvent.change(screen.getByLabelText('Custom volume (mL)'),{target:{value:'0'}})
 expect(ml).toBeDisabled()
 fireEvent.change(screen.getByLabelText('Custom volume (mL)'),{target:{value:'700'}})
 await user.click(screen.getByRole('button',{name:'Record Drink'}))
 await waitFor(()=>expect(onSave).toHaveBeenCalledOnce())
 expect(onSave.mock.calls[0][0]).toMatchObject({servingVolumeMl:700,amountConsumed:10})
})
it('caps plus controls at the boundary without clamping typed overflow', async()=>{
 const {user}=await setup()
 fireEvent.change(screen.getByLabelText('Number of servings consumed'),{target:{value:'9.9'}})
 await user.click(screen.getByRole('button',{name:'Increase servings'}))
 expect(screen.getByLabelText('Number of servings consumed')).toHaveValue(10)
 fireEvent.change(screen.getByLabelText('Number of servings consumed'),{target:{value:'11'}})
 expect(screen.getByLabelText('Number of servings consumed')).toHaveValue(11)
 expect(screen.getByText(/Enter no more than 10 servings/)).toBeInTheDocument()
})
